...
const addsPlayersScoreElToScorecard = (col) => {
    let html = ''
    state.players.forEach(player => {
        html += `<p class="in-game__scorecard-number-score" id="player${player.id}-score-${col}">0</p>`
    })
    return html
}
...
const updatesPlayerScoreToScorecard = (id, target) => {
    ...
    const targetScore = state.players[+id - 1].targets[target - 1].score

    scoreEl.innerText = targetScore === null ? '0' : targetScore
}
...
window.addEventListener('load', () => {
   if (storedStateString) {
    try {
        setState(JSON.parse(storedStateString))
        setsScreen()
        addPlayerToScorecard()
